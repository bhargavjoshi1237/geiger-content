...
// public.roles and content.role_grants. Pure: validate, console.error on
...
// SHARED suite-wide (public.roles) so a role named "Editor" means one thing
// everywhere, while grants are per-product (content.role_grants) so adopting
...
function contentClient() {
  return createClient().schema("content");
}
...
export async function ensureMembership(projectId, defaultRoleId = null) {
  if (!projectId || !isSupabaseConfigured()) return null;
  try {
    const { data, error } = await contentClient().rpc("rbac_ensure_membership", {
...
// Assign a role. Look-then-write rather than upsert: the (project, user, role)
// unique index is PARTIAL (`where deleted_at is null`), and ON CONFLICT cannot
// infer a partial index through PostgREST. Re-granting a role somebody
// previously held revives that row, which also clears its deleted_at.
...
// Read-only permission tester (Governance → Permissions): evaluates one user's
// grants against a capability via the content.rbac_evaluate RPC and returns the
// winning grant path — { allowed, roleId, roleName, patterns, scope } or null.
export async function evaluatePermission(projectId, userId, permissionKey) {
  if (!projectId || !userId || !permissionKey || !isSupabaseConfigured()) {
    return null;
  }
  try {
    const { data, error } = await contentClient().rpc("rbac_evaluate", {
      p_project_id: projectId,
      p_user_id: userId,
      p_permission: permissionKey,
    });
    if (error) {
      console.error("[rbac.evaluatePermission]", error.message);
      return null;
    }
    ...
    return {
      ...
      roleId: row.matched_role_id ?? null,
      roleName: row.matched_role_name ?? null,
      patterns: Array.isArray(row.matched_patterns) ? row.matched_patterns : [],
      scope: row.scope && typeof row.scope === "object" ? row.scope : {},
    };
  } catch (e) {
    console.error("[rbac.evaluatePermission]", e);
    return null;
  }
}
