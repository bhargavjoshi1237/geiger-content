...
> **This product's schema is `content`.** Its migration ledger is
> `content.geiger_migrations`. Substitute `content` wherever this doc writes
...
(`content.tickets`), never `content.flow_tickets`.
...
-- Add seat maps
--
-- Owns content.seat_maps. Free-form header: what this changes and why.

-- @up
create table if not exists content.seat_maps (
...
-- @down
drop table if exists content.seat_maps;
```
...
create table if not exists content.tickets (...);
alter table content.tickets add column if not exists tier text;   -- back-fill older copies
create index if not exists tickets_event_idx on content.tickets (event_id);

drop trigger if exists tickets_touch_updated_at on content.tickets;
create trigger tickets_touch_updated_at
before update on content.tickets
for each row execute function content.touch_updated_at();

drop policy if exists tickets_demo_all on content.tickets;
create policy tickets_demo_all on content.tickets for all to anon, authenticated
...
```sql
alter table content.tickets enable row level security;
drop policy if exists tickets_demo_all on content.tickets;
create policy tickets_demo_all on content.tickets for all to anon, authenticated
  using (true) with check (true);
```
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on content.tickets (event_id);
```
